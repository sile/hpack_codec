...
fn request_examples_without_huffman_coding() {
    ...
    {
        let mut block = track_try_unwrap!(decoder.enter_header_block(&encoded_data[..]));
    ...
    }
    {
        let mut block = track_try_unwrap!(decoder.enter_header_block(&encoded_data[..]));
        assert_decode!(block, b":method", b"GET");
        assert_decode!(block, b":scheme", b"https");
...
fn response_examples_without_huffman_coding() {
    ...
    {
        let mut block = track_try_unwrap!(decoder.enter_header_block(&encoded_data[..]));
        assert_decode!(block, b":status", b"302");
    ...
    {
        let mut block = track_try_unwrap!(decoder.enter_header_block(&encoded_data[..]));
        assert_decode!(block, b":status", b"307");
        assert_decode!(block, b"cache-control", b"private");
        assert_decode!(block, b"date", b"Mon, 21 Oct 2013 20:13:21 GMT");
        assert_decode!(block, b"location", b"https://www.example.com");
        assert_eob!(block);
    ...
    {
        let mut block = track_try_unwrap!(decoder.enter_header_block(&encoded_data[..]));
        assert_decode!(block, b":status", b"200");
        assert_decode!(block, b"cache-control", b"private");
        assert_decode!(block, b"date", b"Mon, 21 Oct 2013 20:13:22 GMT");
        assert_decode!(block, b"location", b"https://www.example.com");
        assert_decode!(block, b"content-encoding", b"gzip");
        assert_decode!(
            block,
            b"set-cookie",
